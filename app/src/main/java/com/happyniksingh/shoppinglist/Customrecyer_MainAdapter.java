...
public class Customrecyer_MainAdapter extends RecyclerView.Adapter<Customrecyer_MainAdapter.Myviewholder>{

    List<ShopsCart>arraylist;
    private ShpCartEventListener listener;
    Context context;
    private boolean multiCheckMode = false;
    ShopCartDao shopCartDao;

    public Customrecyer_MainAdapter(Context context) {
    ...
    public Myviewholder onCreateViewHolder(@NonNull ViewGroup viewGroup, int i) {
        View view= LayoutInflater.from(viewGroup.getContext()).inflate(R.layout.mainshopitems1,viewGroup,false);
        return new Myviewholder(view);
    }

    @Override
    public void onBindViewHolder(@NonNull final Myviewholder myviewholder, int i) {
        final ShopsCart shopsCart = arraylist.get(i);
       ...
       String[]colors={
        ...
        gradientDrawable.setColor(Color.parseColor ("#"+colors[new Random().nextInt(12)]));
        myviewholder.cardView.setBackground(gradientDrawable);
        myviewholder.textViewshopitemname.setText(shopsCart.getNameOfItem());
        AssetManager assetManager=context.getAssets();
        Typeface typeface=Typeface.createFromAsset(assetManager,"fonts/Arkhip_font.ttf");
        myviewholder.textViewshopitemname.setTypeface(typeface);
        String itemno=String.valueOf(shopsCart.getId());
        myviewholder.textViewoption.setTypeface(typeface);
        myviewholder.textViewoption.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(final View v) {
               listener.onoptionclik(shopsCart,myviewholder.textViewoption);
            }
        });
        myviewholder.itemView.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                listener.onNoteClick(shopsCart);
            }
        });
        myviewholder.itemView.setOnLongClickListener(new View.OnLongClickListener() {
            @Override
            public boolean onLongClick(View view) {
                listener.onNoteLongClick(shopsCart);
                return false;
            }
        });

        if (multiCheckMode) {
            myviewholder.checkBox.setVisibility(View.VISIBLE); // show checkBox if multiMode on
            myviewholder.checkBox.setChecked(shopsCart.isChecked());
        } else myviewholder.checkBox.setVisibility(View.GONE);

    }
    public List<ShopsCart> getCheckedNotes() {
        List<ShopsCart> checkedNotes = new ArrayList<>();
        for (ShopsCart n : this.arraylist) {
            if (n.isChecked())
                checkedNotes.add(n);
        }
    ...
    public void setNotes(List<ShopsCart> list){
        this.arraylist=list;
    ...
    public class Myviewholder extends RecyclerView.ViewHolder {
        TextView textViewshopitemname;
        private CheckBox checkBox;
        public TextView textViewoption;
        CardView cardView;
        public Myviewholder(@NonNull View itemView) {
            super(itemView);
            textViewshopitemname=itemView.findViewById(R.id.tvname);
            cardView=itemView.findViewById(R.id.cardview);
            checkBox = itemView.findViewById(R.id.checkBox);
            textViewoption = itemView.findViewById(R.id.textViewOptions);
        }
    }
    public void setListener(ShpCartEventListener listener) {
        this.listener = listener;
    }
    public void setMultiCheckMode(boolean multiCheckMode) {
        this.multiCheckMode = multiCheckMode;
        if (!multiCheckMode)
            for (ShopsCart note : this.arraylist) {
